...
export class GenericCartesian extends Element {
    ...
    walkFrames() {
        ...
        const initialModel = new CartesianGrammar({
        ...
        const relativeModel = this.decorators
            .filter(x => x)
            .reduce((model, transform) => CartesianGrammar.compose(model, transform(model, args)), initialModel);

        const adjustedModel = (config.adjustPhase ? this.adjusters : [])
            .filter(x => x)
            .reduce((model, transform) => CartesianGrammar.compose(model, transform(model, args)), relativeModel);
